...
function usage() {
  console.log([
    "Usage:",
    '  node scripts/export_ppp_record.mjs --game-dir "<PQTC dir>" [--out-root "<dir>"] "<recordName>" [...]',
  ].join("\n"));
}
...
function parseArgs(argv) {
  ...
  if (names.length === 0) throw new Error("Missing record name");
...
const { allPath, outRoot, names } = parseArgs(process.argv.slice(2));
...
function xorIndexAcf(buf, recordIndex) {
...
function decodeHeaderBlock(encrypted) {
  const buf = Buffer.from(encrypted);
  for (let i = 0; i < 3; i++) {
    writeU32(buf, i * 4, (readU32(buf, i * 4) - keyAt(13 + i)) >>> 0);
  }
...
function readHeader() {
  ...
  try {
    ...
    fs.readSync(fd, encrypted, 0, encrypted.length, manifest.headerOffset ?? 0xe5);
    const decoded = decodeHeaderBlock(encrypted);
    return {
...
function decodeIndexRecord(raw, recordIndex) {
  ...
  for (let i = 0; i < 0x20; i++) {
    const off = 8 + i * 4;
    writeU32(buf, off, (readU32(buf, off) - keyAt(0x80 + i)) >>> 0);
  }
  const local = indexLocalByte(lo, hi);
  for (let p = 0; p < recSize; p += 4) buf[p] ^= local;
  writeU32(buf, 0, lo);
  writeU32(buf, 4, hi);
  xorIndexAcf(buf, recordIndex);
  return buf;
}
...
function readAllIndexRecords() {
  ...
  try {
    ...
    for (let i = 0; i < header.fileNum; i++) {
      ...
      const decoded = decodeIndexRecord(raw, i);
      out.push({
        index: i,
...
function cryptTextBody(buf, size2) {
  const out = Buffer.from(buf);
  for (let p = 0; p + 4 <= out.length; p += 4) {
    const k = keyAt(size2 + (p >>> 2));
    writeU32(out, p, (readU32(out, p) - k) >>> 0);
  }
  return out;
}

function decodePlain(rec) {
  const fd = fs.openSync(allPath, "r");
  try {
    ...
    return cryptTextBody(encryptedBody, rec.size2);
  } finally {
    fs.closeSync(fd);
  }
}
...
function outPathFor(name) {
  return path.join(outRoot, ...name.split("\\"));
}

const all = readAllIndexRecords();
