...
const outDir = path.resolve(import.meta.dirname, "..", "metadata");
const outPath = path.join(outDir, "localized-records.txt");
const manifestPath = path.join(projectRoot, "tools", "pqtc_text_manifest.json");
const backupPath = path.join(projectRoot, "all.ppp.bak");
const extractedRoot = path.join(projectRoot, "extracted_texts");
...
function localPathForRecord(name) {
  return path.join(extractedRoot, ...name.split("\\"));
}

function normalizeForCompare(buf, ext) {
  if (ext === ".json" || ext === ".txt" || ext === ".sb") {
...
function main() {
  ...
  for (const rec of records) {
    if (!(rec.name.startsWith("Texts\\") || rec.name.startsWith("Doc\\"))) continue;
    const localPath = localPathForRecord(rec.name);
    if (!fs.existsSync(localPath)) continue;
    const ext = path.extname(localPath).toLowerCase();
    if (![".json", ".txt", ".sb"].includes(ext)) continue;
    const local = normalizeForCompare(fs.readFileSync(localPath), ext);
    const backup = normalizeForCompare(decodePlain(manifest, rec), ext);
    if (!local.equals(backup)) changed.push(rec.name);
  }

  fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(outPath, changed.join("\n") + "\n", "utf8");
  console.log(`Wrote ${changed.length} localized records to ${outPath}`);
}
